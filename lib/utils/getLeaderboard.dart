...
const url = 'https://hbchess.app/api/teams/all?limit=25&offset=0';
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < data.length; i++) {
        allTeams.add(Team(
            id: data[i]['_id'],
            sender: User(
                id: data[i]['sender']['_id'],
                username: data[i]['sender']['username']),
            recipient: User(
                id: data[i]['recipient']['_id'],
                username: data[i]['recipient']['username']),
            wins: data[i]['wins'],
            draws: data[i]['ties'],
            loss: data[i]['losses']));
      }
